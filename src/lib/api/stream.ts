...
function validateTestFile(filename: string): boolean {
  let status = false;
...
function getTestFileName(name: string): string {
  let modifiedName = "";
...
class TestFileSystem {
  ...
  public createDir(): Promise<string[]> {
    return (this.destination()
      .then((/* response */) => {
        const promises = [
          this.layout(), // create html file
          this.script(), // create index.js file
          this.testScript(this._dirname), // create *.test.js file
  ...
  public removeDir(): Promise<string> {
    ...
    return (Promise.reject(
      new UsageError(`'${this._dirname}' module doesn't exists under '/csunit/tests/' directory. Nothing to remove.`)
    ));
  }

  public createFiles(filenames: string[]): Promise<string[]> {
    const promises = filenames.map((filename) => {
      if (validateTestFile(filename)) {
        ...
        return (Promise.resolve(
          `${modifiedName} already exists inside ${this._destination}, command will not create the file.`
        )
        );
      }
  ...
  public removeFiles(filenames: string[]): Promise<string[]> {
    const promises = filenames.map((filename) => {
      if (validateTestFile(filename)) {
        const modifiedName = getTestFileName(filename);
        const dest = this.genTestFilePath(modifiedName);
        ...
        return (Promise.resolve(
          `${modifiedName} doesn't exists inside ${this._destination}, command unable to remove file.`
        ));
      }
  ...
  private genTestFilePath(name: string): string {
    let testFilePath = "";
  ...
  private destination(): Promise<boolean> {
...
Please delete the folder before proceed or change -d <dirname>.`)));
  ...
  private layout(): Promise<string> {
    const dirname = this._dirname;

    function middleware(data: string): string {
  ...
  private testScript(filename: string): Promise<string> {
    function middleware(data: string): string {
      // update information.
  ...
  private async updateTestModules(callback: ((content: string) => string)): Promise<string> {
  ...
  private addModule(content: string): string {
    let updated = "";
  ...
  private removeModule(content: string): string {
    let updated = "";
  ...
  // private methods
  private _create(source: string, destination: string, middleware?: (data: string) => string): Promise<string> {
    if (this._destination) {
...
const OStream = {
  createTestDir(dirname: string): Promise<string[]> {
  ...
  createTestFiles(dirname: string, filenames: string[]): Promise<string[]> {
    if (dirname && dirname.length > 0) {
      if (filenames && filenames.length > 0) {
        ...
        return (Promise.reject(
          new UsageError(`'${dirname}' test folder doesn't exists. Please add test folder before adding test script.`)
        ));
      }
  ...
  removeTestDir(dirname: string): Promise<string> {
    if (dirname && dirname.length > 0) {
      const tfs = new TestFileSystem(dirname);
  ...
  removeTestFiles(dirname: string, filenames: string[]): Promise<string[]> {
    if (dirname && dirname.length > 0) {
      if (filenames && filenames.length > 0) {
        const tfs = new TestFileSystem(dirname);
        if (tfs.exists()) {
        ...
        return (Promise.reject(
          new TypeError(`'${dirname}' test folder doesn't exists. Please add test folder before adding test script.`)
        ));
      }
