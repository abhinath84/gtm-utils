...
function mkProjSubDirs(projPath) {
    const projSubDirs = [
        ".gtdata",
        ".gtsub",
        "data",
...
function updateHomeGtconfig(name) {
    //
    ...
    if (homeDir) {
        ...
        return (Promise.reject(new UsageError("'HOME' environment variable either not a directory or doesn't exists the directory.")));
    }
    return (Promise.reject(new UsageError("'HOME' environment variable is not set, please set it before proceed further.")));
}
function createEmptyFiles(gtdataPath) {
...
${cmdMsg}`)));
    ...
    _createPrjinfoFile(name, gtdataPath) {
    ...
    _createPrjinfoJsonFile(gtdataPath) {
        const content = `{
  ...
  "git_link": ""
    ...
    async _copyRecursively(source, destination) {
...
const UIGTMSupport = {
    createProject(name, refSystem) {
        const uigtm = new UIGtm(refSystem);
        return (uigtm.create(name));
    },
    updateProject(name, refSystem) {
        const uigtm = new UIGtm(refSystem);
        return (uigtm.update(name));
    }
};
export { UIGTMSupport };
//# sourceMappingURL=uigtm.js.map